...
import { createGroup, createStudent, createUser } from "../../../../lib/services/management.service";
...
export async function POST(request:Request){try{const user=await requireAdmin(),body=await request.json(),kind=kindOf(body.kind),actor={userId:user.userId,role:user.role};let item;if(kind==="parents"||kind==="teachers")item=await createUser({...body.data,role:kind==="parents"?"PARENT":"TEACHER"},actor);else if(kind==="students")item=await createStudent(body.data,actor);else item=await createGroup(body.data,actor);return Response.json({ok:true,item},{status:201});}catch(error){return apiError(error)}}
