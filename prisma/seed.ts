...
async function main() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    for (let k = 0; k < skillIds.length; k++) await prisma.monthlySkillScore.upsert({ where: { assessmentId_skillCategoryId: { assessmentId, skillCategoryId: skillIds[k] } }, update: {}, create: { assessmentId, skillCategoryId: skillIds[k], score: 70 + ((i + k) % 25), teacherComment: "Good monthly progress" } });
