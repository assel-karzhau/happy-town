...
const nav: Record<Role, NavItem[]> = {
  ...
  teacher: [
    ["Главная", "home"], ["Мои группы", "groups"], ["Посещаемость", "attendance"], ["Добавить урок", "lesson"],
    ["Домашние задания", "homework"], ["Слова", "words"], ["Тесты", "tests"], ["Оценки", "grades"],
    ["Ежемесячный отзыв", "feedback"], ["История ученика", "history"],
...
function Topbar({ role, title, onMenu }: { role: Role; title: string; onMenu: () => void }) {
  return <header className="topbar"><button className="icon-btn menu-trigger" onClick={onMenu} aria-label="Открыть меню"><Menu /></button><div><p className="eyebrow">Электронный дневник</p><h1>{title}</h1></div><div className="top-actions"><RoleSwitcher role={role} /><button className="icon-btn notification" aria-label="Уведомления"><Bell size={19} /><i /></button><div className="profile-chip"><span className="avatar">МС</span><b>{role === "parent" ? "Мама Амины" : role === "teacher" ? "Айгуль Сериковна" : "Администратор"}</b><ChevronDown size={15} /></div></div></header>;
}
...
function ProgressPage() {
  ...
  return <><div className="tabs">{["6 месяцев","3 месяца","Месяц","Всё время"].map(x=><button onClick={()=>setPeriod(x)} className={period===x?"active":""} key={x}>{x}</button>)}</div><SectionCard title="Прогресс по навыкам" action={<StatusBadge>{period}</StatusBadge>}><div className="chart-legend">{[["#ff1028","Speaking"],["#ff8a00","Listening"],["#2878ee","Reading"],["#19a445","Writing"],["#8b3fd1","Vocabulary"]].map(([c,n])=><span key={n}><i style={{background:c}}/>{n}</span>)}</div><div className="chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={progressData} margin={{top:10,right:10,left:-22,bottom:0}}><CartesianGrid stroke="#eef0f3" vertical={false}/><XAxis dataKey="month" tickLine={false} axisLine={false}/><YAxis domain={[0,10]} tickLine={false} axisLine={false}/><Tooltip/><Line type="monotone" dataKey="Speaking" stroke="#ff1028" strokeWidth={3}/><Line type="monotone" dataKey="Listening" stroke="#ff8a00" strokeWidth={2}/><Line type="monotone" dataKey="Reading" stroke="#2878ee" strokeWidth={2}/><Line type="monotone" dataKey="Writing" stroke="#19a445" strokeWidth={2}/><Line type="monotone" dataKey="Vocabulary" stroke="#8b3fd1" strokeWidth={2}/></LineChart></ResponsiveContainer></div></SectionCard><div className="two-col"><SectionCard title="Последние достижения"><div className="achievement"><Star/><div><b>Лучший результат в тесте</b><p>Unit 2 Test — 82%</p></div></div><div className="achievement"><GraduationCap/><div><b>Изучено новых слов</b><p>34 слова за май</p></div></div></SectionCard><SectionCard title="Фокус на следующий месяц"><p className="muted">Продолжить развивать уверенную устную речь и уделить больше внимания письменным заданиям.</p><div className="progress-bar"><i style={{width:"68%"}}/></div><small>Общий прогресс · 68%</small></SectionCard></div></>;
}

function TeacherHome() { return <><div className="welcome"><div><p>Добрый день, Айгуль Сериковна!</p><h2>Сегодня у вас 3 занятия</h2><span>Первое занятие в 10:00 · Kids Starter</span></div><Button>Открыть расписание</Button></div><div className="stats-grid"><StatCard icon={UsersRound} label="Мои группы" value="3" note="активные группы"/><StatCard icon={GraduationCap} label="Ученики" value="24" note="всего учеников"/><StatCard icon={CalendarDays} label="Занятия" value="18" note="в этом месяце"/><StatCard icon={MessageSquareText} label="Отзывы" value="4" note="нужно заполнить" color="orange"/></div><SectionCard title="Быстрые действия"><div className="quick-grid"><Link href="/teacher/attendance"><CalendarDays/>Отметить посещаемость</Link><Link href="/teacher/lesson"><Plus/>Добавить урок</Link><Link href="/teacher/tests"><Star/>Результат теста</Link><Link href="/teacher/feedback"><MessageSquareText/>Написать отзыв</Link></div></SectionCard></> }

function GroupsPage() { return <><FilterBar/><div className="group-grid">{groups.map((g)=><SectionCard key={g[0]}><div className="group-icon"><UsersRound/></div><StatusBadge>{g[1]}</StatusBadge><h3>{g[0]}</h3><p>{g[2]} · {g[3]}</p><div className="progress-label"><span>{g[4]}</span><b>{g[5]}</b></div><div className="progress-bar"><i style={{width:g[5]}}/></div><Link href="/teacher/group">Открыть группу <ChevronRight size={15}/></Link></SectionCard>)}</div></> }

function FormPage({ kind }: { kind: string }) {
  const [saved,setSaved]=useState(false);
  const fields: Record<string,string[]> = { lesson:["Дата урока","Группа","Учебник","Раздел","Тема урока","Что изучали","Грамматика","Новые слова","Домашнее задание","Комментарий учителя"], tests:["Ученик","Тест","Раздел","Дата","Максимальный балл","Набранный балл","Speaking","Listening","Комментарий"], feedback:["Ученик","Месяц","Достижения","Что необходимо улучшить","Рекомендации родителям","Общий комментарий","Уровень прогресса"], homework:["Группа","Дата","Срок","Учебник и страница","Упражнение","Описание"] };
  const title = kind === "lesson" ? "Данные проведённого урока" : kind === "tests" ? "Новый результат теста" : kind === "feedback" ? "Ежемесячный отзыв" : "Новое домашнее задание";
  return <SectionCard title={title} action={saved?<StatusBadge>Сохранено</StatusBadge>:undefined}><form className="form-grid" onSubmit={(e)=>{e.preventDefault();setSaved(true)}}>{(fields[kind]??fields.lesson).map((field,i)=><label key={field} className={i>4?"full":""}><span>{field}{i<5&&<em>*</em>}</span>{i>4?<textarea placeholder={`Введите: ${field.toLowerCase()}`} required={i<5}/>:<input placeholder={`Выберите или введите`} required={i<5}/>}<small>{i===0?"Обязательное поле":""}</small></label>)}<div className="form-actions"><Button secondary>Отмена</Button><Button type="submit">Сохранить</Button></div></form></SectionCard>;
}

function FeedbackPage() { return <div className="feedback-list">{[["Май 2025","Амина стала увереннее отвечать на вопросы и использовать новые слова в предложениях.","Нужно уделить больше внимания письменным заданиям и правописанию."],["Апрель 2025","Хороший прогресс в чтении и словарном запасе.","Рекомендуется больше практиковать устную речь."],["Март 2025","Ребёнок активно участвует в уроках.","Нужно повторять новые слова после каждого занятия."]].map(([month,good,improve])=><SectionCard key={month} title={month} action={<StatusBadge>Хороший прогресс</StatusBadge>}><p className="label">Достижения</p><p>{good}</p><p className="label">Что улучшить</p><p>{improve}</p><div className="teacher-sign"><span className="avatar">МС</span><div><b>Максим Станиславов</b><small>10 мая 2025</small></div></div><a href="#">Читать полностью <ChevronRight size={14}/></a></SectionCard>)}</div> }

function HistoryPage() { return <SectionCard><div className="timeline">{[["15 мая 2025","Завершён раздел My World","Пройдено 5 тем, итоговый тест — 82%"],["1 мая 2025","Ежемесячный отчёт","Хороший прогресс по Speaking и Vocabulary"],["2 фев 2025","Смена учебника","Начат Family and Friends 1"],["10 янв 2025","Перевод в Kids Starter","Новый преподаватель — Айгуль Сериковна"],["1 сен 2024","Начало обучения","Уровень Starter"]].map(([date,title,text],i)=><div key={date}><i>{i===0?<Star size={16}/>:<Check size={16}/>}</i><time>{date}</time><h3>{title}</h3><p>{text}</p></div>)}</div></SectionCard> }

function ProfilePage() { return <div className="two-col"><SectionCard title="Данные родителя"><div className="profile-hero"><span className="avatar big">МС</span><div><h3>Мама Амины</h3><p>Родитель · 2 ребёнка</p></div></div><dl className="profile-dl"><div><dt>Телефон</dt><dd>+7 701 524 1191</dd></div><div><dt>Email</dt><dd>serikova@example.com</dd></div><div><dt>Язык интерфейса</dt><dd>Русский</dd></div></dl><Button secondary>Редактировать данные</Button></SectionCard><SectionCard title="Дети"><div className="child-profile"><div className="student-avatar small">АС</div><div><h3>{currentStudent.name}</h3><p>{currentStudent.group} · {currentStudent.level}</p><small>{currentStudent.teacher}</small></div><ChevronRight/></div><div className="child-profile"><div className="student-avatar small blue">ДС</div><div><h3>Данияр Сериков</h3><p>Kids A1 · Beginner</p><small>Максим Станиславов</small></div><ChevronRight/></div></SectionCard></div> }
...
function GenericDataPage({ type }: { type: string }) {
  if(type==="words") return <><FilterBar/><SectionCard><DataTable headers={["Слово","Перевод","Тема","Статус"]} rows={words}/></SectionCard></>;
  if(type==="tests") return <><FilterBar search={false}/><SectionCard><DataTable headers={["Название теста","Раздел","Дата","Результат","Процент"]} rows={tests}/></SectionCard></>;
  if(type==="homework") return <><FilterBar/><SectionCard><DataTable headers={["Дата","Срок","Учебник","Задание","Статус"]} rows={homework}/></SectionCard></>;
...
function PageContent({ role, page }: { role: Role; page: string }) {
  ...
  if(role==="teacher") {
    if(page==="home") return <TeacherHome/>; if(page==="groups"||page==="group") return <GroupsPage/>; if(page==="attendance") return <AttendancePage editable/>; if(["lesson","feedback"].includes(page)) return <FormPage kind={page}/>; if(page==="history") return <HistoryPage/>; if(page==="tests") return <FormPage kind="tests"/>; if(page==="homework") return <><FormPage kind="homework"/><GenericDataPage type="homework"/></>; return <GenericDataPage type={page}/>;
  }
